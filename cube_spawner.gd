...
func _process(delta):
	...
	for i in range(cubes.size() - 1, -1, -1):
		var cube = cubes[i]
		if cube == null or !is_instance_valid(cube):
			# Remove the cube from the list if it's already freed
			cubes.erase(cube)
			continue
...
func spawn_cube():
	...
	if collision_shape != null:
		collision_shape.set_collision_layer(1 << 10)  # Layer 10 for blue cubes
